package com.cp.food.demoFood.controller;
...
import com.cp.food.demoFood.service.RecipeService;
import com.cp.food.demoFood.entity.Recipe;
import com.cp.food.demoFood.exception.RecipeNotFoundException;
import com.cp.food.demoFood.repository.RecipeRepository;
...
public class RecipeController {
	...
	public ResponseEntity<?> createRecipe(@RequestBody Recipe recipe) throws RecipeNotFoundException{
		...
		if (opt.isPresent()) {
			throw new RecipeNotFoundException("Recipe is already existed");

		} else {

			recipeService.addRecipe(recipe);

			return new ResponseEntity<>(" Recipe added in the list ", HttpStatus.CREATED);
